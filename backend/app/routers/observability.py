...
async def list_ingestion_tasks(limit: int = 50):
    async with AsyncSessionLocal() as session:
        result = await session.execute(
            text(
                """
                SELECT
                    t.id, t.paper_id, COALESCE(p.title, t.file_name) AS title,
                    t.file_name, COALESCE(p.status, t.stage) AS status,
                    t.stage, t.progress, t.error_msg, t.started_at, t.created_at
                FROM ingest_tasks t
                LEFT JOIN papers p ON p.id = t.paper_id AND p.user_id = t.user_id
                WHERE t.user_id = :user_id
                ORDER BY t.created_at DESC, t.id DESC
                LIMIT :limit
                """
            ),
            {"user_id": MOCK_USER_ID, "limit": limit},
        )
...
async def list_observability_query_logs(limit: int = 50, offset: int = 0):
    async with AsyncSessionLocal() as session:
        result = await session.execute(
            text(
                """
            ...
            {"user_id": MOCK_USER_ID, "limit": limit, "offset": offset},
        )
        logs = []
...
async def list_query_logs(limit: int = 10, offset: int = 0):
    return await list_observability_query_logs(limit=limit, offset=offset)
