...
- [决策] Docling 作为默认 PDF 版面解析 Provider，GROBID 作为默认论文元数据/参考文献解析 Provider，MinerU 仅保留显式配置回退 —— MinerU 云端 Pipeline 易受服务侧配置影响；Docling 本地解析优先保障 `doc_blocks -> indexing -> chat` 溯源链路，GROBID 提供结构化 TEI 元数据与引用边，LLM 仅作为 GROBID 失败时的引用抽取降级。
